      ...
      this.$http({
		...
		if(json.shouji!=''&&json.shouji){
          ...
          this.$message.error(data.msg);
            ...
            this.chimaOptions = "S,M,XL,XLL,XLLL".split(',')
    },
    // 多级联动参数
    info(id) {
      this.$http({
        url: `xuangoufuzhuang/info/${id}`,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `xuangoufuzhuang/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.xuangoufuzhuangCrossAddOrUpdateFlag = false;
						 this.parent.search();
