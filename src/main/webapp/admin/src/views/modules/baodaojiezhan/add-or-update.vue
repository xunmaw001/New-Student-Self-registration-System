    ...
    >
      ...
      <el-col :span="12">
        ...
        <div v-else>
          ...
          <el-row>
            <el-col :span="24">
              <el-form-item v-if="type!='info'"  label="报道流程" prop="baodaoliucheng">
                <editor 
                    style="min-width: 200px; max-width: 600px;"
                    v-model="ruleForm.baodaoliucheng" 
                    class="editor" 
                    action="file/upload">
                </editor>
              </el-form-item>
              <div v-else>
                <el-form-item v-if="ruleForm.baodaoliucheng" label="报道流程" prop="baodaoliucheng">
                    <span v-html="ruleForm.baodaoliucheng"></span>
                </el-form-item>
              </div>
            </el-col>
          </el-row>
...
export default {
  ...
  methods: {
	...
	this.ruleForm.baodaoliucheng = this.ruleForm.baodaoliucheng.replace(reg,'../../../ssmg48yd/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `baodaojiezhan/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.baodaojiezhanCrossAddOrUpdateFlag = false;
						 this.parent.search();
