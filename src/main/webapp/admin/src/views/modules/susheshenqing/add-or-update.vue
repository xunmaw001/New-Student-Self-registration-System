<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'"  label="床位数" prop="chuangweishu">
          <el-select :disabled="ro.chuangweishu" v-model="ruleForm.chuangweishu" placeholder="请选择床位数">
            <el-option
                v-for="(item,index) in chuangweishuOptions"
                v-bind:key="index"
                :label="item"
                :value="item">
            </el-option>
        ...
        <div v-else>
          <el-form-item class="input" label="床位数" prop="chuangweishu">
	      <el-input v-model="ruleForm.chuangweishu"
                placeholder="床位数" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
      ...
      this.$http({
		...
		if(json.shouji!=''&&json.shouji){
          ...
          this.$message.error(data.msg);
            ...
            this.chuangweishuOptions = "1".split(',')
    },
    // 多级联动参数
    info(id) {
      this.$http({
        url: `susheshenqing/info/${id}`,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
...
var objcross = this.$storage.getObj('crossObj');
      ...
      if(objcross!=null) {
	      objcross.chuangweishu = objcross.chuangweishu - this.ruleForm.chuangweishu
	      if(objcross.chuangweishu<0){
		this.$message.error("床位数不足");
		return
	      }
                }
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `susheshenqing/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.susheshenqingCrossAddOrUpdateFlag = false;
						 this.parent.search();
