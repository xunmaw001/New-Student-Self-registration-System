<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="学校位置" prop="xuexiaoweizhi">
          <el-input v-model="ruleForm.xuexiaoweizhi" 
              placeholder="学校位置" clearable  :readonly="ro.xuexiaoweizhi"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="学校位置" prop="xuexiaoweizhi">
              <el-input v-model="ruleForm.xuexiaoweizhi" 
                placeholder="学校位置" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="24">  
        ...
        <div v-else>
          ...
          <el-row>
            <el-col :span="24">
              <el-form-item class="textarea" v-if="type!='info'" label="路过公交" prop="luguogongjiao">
                <el-input
                  style="min-width: 200px; max-width: 600px;"
                  type="textarea"
                  :rows="8"
                  placeholder="路过公交"
                  v-model="ruleForm.luguogongjiao" >
                </el-input>
              </el-form-item>
              <div v-else>
                <el-form-item v-if="ruleForm.luguogongjiao" label="路过公交" prop="luguogongjiao">
                    <span>{{ruleForm.luguogongjiao}}</span>
                </el-form-item>
              </div>
            </el-col>
          </el-row>
          <el-row>
            <el-col :span="24">
              <el-form-item class="textarea" v-if="type!='info'" label="附近建筑" prop="fujinjianzhu">
                <el-input
                  style="min-width: 200px; max-width: 600px;"
                  type="textarea"
                  :rows="8"
                  placeholder="附近建筑"
                  v-model="ruleForm.fujinjianzhu" >
                </el-input>
              </el-form-item>
              <div v-else>
                <el-form-item v-if="ruleForm.fujinjianzhu" label="附近建筑" prop="fujinjianzhu">
                    <span>{{ruleForm.fujinjianzhu}}</span>
                </el-form-item>
              </div>
            </el-col>
          </el-row>
          <el-row>
            <el-col :span="24">
              <el-form-item v-if="type!='info'"  label="交通指南" prop="jiaotongzhinan">
                <editor 
                    style="min-width: 200px; max-width: 600px;"
                    v-model="ruleForm.jiaotongzhinan" 
                    class="editor" 
                    action="file/upload">
                </editor>
              </el-form-item>
              <div v-else>
                <el-form-item v-if="ruleForm.jiaotongzhinan" label="交通指南" prop="jiaotongzhinan">
                    <span v-html="ruleForm.jiaotongzhinan"></span>
                </el-form-item>
              </div>
            </el-col>
          </el-row>
...
export default {
  ...
  methods: {
	...
	this.ruleForm.jiaotongzhinan = this.ruleForm.jiaotongzhinan.replace(reg,'../../../ssmg48yd/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `jiaotongzhiyin/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.jiaotongzhiyinCrossAddOrUpdateFlag = false;
						 this.parent.search();
